...
| UAT-031 | UAT | Remove Import Puzzle -> Photo. | High | Complete for next UAT | Removed the Photo tab, image picker dependency, and iOS/Android camera/photo permissions. Import returns to Paste and Grid only while OCR/photo import is deferred for a proper design spike. | Pending next UAT build. |
| UAT-032 | UAT | Add search feature in Record Hall. | High | Complete for next UAT | Added Record Hall search across puzzle title, puzzle ID, pack, difficulty, seal, score class, score, time, clean/favorite status, notes, and completion date. Search works together with existing filters. | Pending next UAT build. |
| UAT-033 | Product UX | Redesign navigation flows and Home UX for Tea Moment, Official Ranking, and 7-8 top-level items. | High | Proposal drafted | Created a separate navigation proposal recommending Tea Moment as the calm daily anchor, Official Ranking as a separate official ceremony, and Home sections for Today, Play, Study, and Mastery. | Pending product review. |
...
| IDEA-007 | Content Import | Gate 7 polish updated | See `Orbace Sudoku - External Puzzle Import Solution.md`. Current import scope supports paste string with example CTA and manual grid entry. Photo import was removed after UAT feedback and is deferred until a proper OCR/camera design spike. |
