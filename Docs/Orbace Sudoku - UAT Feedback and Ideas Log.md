...
| UAT-022 | Build 24 Android crash | Android app crashes immediately on launch. iOS build is unaffected. | Critical | Complete | Root cause: `google_mobile_ads` plugin requires `com.google.android.gms.ads.APPLICATION_ID` in `AndroidManifest.xml` at the native layer even when no ads are shown — the GMA SDK throws `IllegalStateException` during plugin registration before Flutter initialises. `AdMobConfig.isSupportedPlatform` correctly gates all Dart ad code to iOS-only, but the native SDK check runs regardless. Fix: added the required meta-data entry with Google's test App ID (`ca-app-pub-3940256099942544~3347511713`) as a placeholder until the real Android AdMob App ID is configured. | Fixed in build `1.0.0 (25)`. |
...
| `1.0.0 (25)` | Android AAB | Android crash fix (UAT-022) | Local validation passed; AAB built | Hotfix: missing AdMob APPLICATION_ID meta-data caused startup crash on all Android devices. |
